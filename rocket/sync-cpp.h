...
*/
	...
	public:
		...
		{
			Track track;
			track.track = (sync_track*)sync_get_track(device, name);
			return track;
		}*/
		
		sync_track* getTrack(const char* name)
		{
			return (sync_track*)sync_get_track(device, name);
		}
		
#ifndef SYNC_PLAYER
		int update(double row, void* timer)
		{
			return sync_update(device, (int)floor(row), &cb, timer);
		}
		
		int connect()
		{
			return sync_connect(device, "localhost", SYNC_DEFAULT_PORT);
		}
#endif
